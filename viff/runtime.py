...
from viff.field import GF256, FieldElement, GF
...
class Share(Deferred):
    ...
    def __init__(self, runtime, field, value=None, fp=False):
        """Initialize a share.
    ...
    def __radd__(self, other):
        """Addition (refelcted argument version)."""
        return self.runtime.add(self, other)
    ...
    def __floordiv__(self, other):
        """Subtraction."""
        return self.runtime.floordiv(self, other)

    def __rfloordiv__(self, other):
        """Subtraction (reflected argument version)."""
        return self.runtime.floordiv(other, self)
    ...
    def set_fp_False(self):
        self.fp = False

    def set_fp_True(self):
        self.fp = False

		

    def clone_nofp(self):
        """Clone a share with fp disabled.
    ...
    def clone_fp(self):
        ...
        def split_result(result):
            clone.callback(result)
            return result
        clone = Share(self.runtime, self.field, None, True)
        self.addCallback(split_result)
        return clone
    ...
    def clone(self):
        ...
        def split_result(result):
            clone.callback(result)
            return result
        clone = Share(self.runtime, self.field, None, self.fp)
        self.addCallback(split_result)
        return clone

def FPShare(runtime, field, value=None):
    """ Macro for creating shares. 
    
    If value is a float, then fp is set, else
    if value is either an int or GFElement then
    fp is not set. """

    if isinstance(value, float): 
        if value !=0 and int(2**runtime.res*value)==0:
            print "Warning (possible loss of data): Nonzero number %f set to zero." %value
        value = field(int(2**runtime.res*value), True)
        return Share(runtime, field, value, True)
    ...
    return Share(runtime, field, value)
...
class ShareList(Share):
    ...
    *c* are ready:
    ...
    def __init__(self, shares, threshold=None):
        ...
        fp = reduce(operator.or_, [share.fp for share in shares])
        Share.__init__(self, shares[0].runtime, shares[0].field, None, fp)
...
class ShareExchanger(Int16StringReceiver):
    ...
    def connectionLost(self, reason):
        if not self.shutting_down: reason.trap(ConnectionDone)
    ...
    def sendShare_shamir(self, program_counter, share):
        """Send a share.

        The program counter and the share are converted to bytes and
        sent to the peer.
        """
        self.sendData(program_counter, SHARE, hex(share))
...
class SelfShareExchanger(ShareExchanger):
    ...
    def __init__(self, myid, factory):
    ...
    def add_options(parser):
        ...
        group.add_option("-l", "--bit-length", type="int", metavar="L",
                         help=("Maximum bit length of input numbers for "
                               "comparisons."))
        group.add_option("-r", "--resolution", type="int", metavar="R",
                         dest = "res", help=("Resolution of fixed pointnumbers"))
        group.add_option("-t", "--threshold", dest="threshold", type="int",
        ...
        group.add_option("-L", "--local", action="store_true", dest="local",
                         help="Perform computation locally with LocalRuntime")
...
#BS-del        assert threshold > 0, "Must use a positive threshold."
        ...
        self.options.bit_length = self.options.bit_length + self.options.res
    ...
    def synchronize(self):
        ...
        """
        self.increment_pc()
        shares = [self._exchange_shares(player, GF(13)(0))
    ...
    def _exchange_shares(self, peer_id, field_element):
        ...
        if peer_id == self.id:
            return Share(self, field_element.field, field_element, field_element.fp)
        else:
            share = self._expect_share(peer_id, field_element.field, field_element.r)
            pc = tuple(self.program_counter)
            self.protocols[peer_id].sendShare(pc, field_element)
            return share

    def _expect_share(self, peer_id, field, r=False):
        share = Share(self, field, None, bool(r))
        share.addCallback(lambda value: field(long(value, 16), r))
        self._expect_data(peer_id, SHARE, share)
        return share

    def _expect_share_shamir(self, peer_id, field,fp=False):
        share = Share(self, field, None, fp)

        def handle_received_share(value):
            val = long(value, 16)
            fp = val & 1 == 1
            return field(val // 2,fp)

        share.addCallback(handle_received_share)
        self._expect_data(peer_id, SHARE, share)
        return share
...
def create_runtime(myid, players, threshold, options=None, runtime_class=None):
    ...
    code like this::
        ...
        pre_runtime = create_runtime(myid, players, 1)
    ...
    from viff.local import LocalRuntime

    if options and options.local:
        runtime_class = LocalRuntime
        
    if runtime_class==LocalRuntime:
        # do not set up any network connections
        result = Deferred()
        runtime = runtime_class(players[myid], threshold, options)
        reactor.callLater(0, result.callback, runtime)
        return result
    ...
    if options and options.profile:
        ...
        def new_run(*args, **kwargs):
            ...
            stats.dump_stats("player-%d.pstats" % myid)
        reactor.run = new_run
    ...
    if options and options.ssl:
        ...
        class SSLContextFactory(ContextFactory):
            def __init__(self, myid):
                """Create new SSL context factory for *myid*."""
                self.id = myid
                ctx = SSL.Context(SSL.SSLv3_METHOD)
                # TODO: Make the file names configurable.
                try:
                    ctx.use_certificate_file('player-%d.cert' % myid)
                    ctx.use_privatekey_file('player-%d.key' % myid)
        ...
        ctx_factory = SSLContextFactory(myid)
