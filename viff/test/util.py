...
class RuntimeTestCase(TestCase):
    ...
    def setUp(self):
        ...
        self.runtimes = []
        for myid in reversed(range(1, self.num_players+1)):
            _, players = load_config(configs[myid])
            self.create_loopback_runtime(myid, players)
    ...
    def create_loopback_runtime(self, myid, players):
        ...
        @param myid: ID of the player owning this Runtime.
        ...
        runtime = self.runtime_class(players[myid], self.threshold)
        ...
        for peer_id in players:
            if peer_id != myid:
                ...
                client_key = (myid, peer_id)
                server_key = (peer_id, myid)
                ...
                protocol = SelfShareExchanger(myid, SelfShareExchangerFactory(runtime))
                protocol.transport = FakeTransport()
                # Keys for when we are the client and when we are the server.
                server_key = (myid, myid)
                # Store a protocol used when we are the server.
                self.protocols[server_key] = protocol     
