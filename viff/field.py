...
class GF256(FieldElement):
    ...
    def __init__(self, value, fp=False):
...
def GF(modulus, resolution=0):
    ...
    class GFElement(FieldElement):
        ...
        def __init__(self, value, fp=None):
            self.value = value % self.modulus
            if not fp:
                self.fp = False
                self.r  = 0
        ...
        def __add__(self, other):
            ...
            try:
                ...
                return GFElement((2**(b*d))*self.value + (2**((1-b)*d))*other.value, (1-b)*self.r + b*other.r)
            except AttributeError:
                return GFElement(self.value + 2**(self.r)*other, self.r)
        ...
        def __rsub__(self, other):
            ...
            return GFElement((2**self.r)*other - self.value, self.r)
        ...
        def __xor__(self, other):
            ...
            try:
                assert self.field is other.field, "Fields must be identical"
                return GFElement(self.value ^ other.value, self.fp or other.fp)
            except AttributeError:
                return GFElement(self.value ^ other, self.fp)

        def __rxor__(self, other):
            """Xor for bitvalues (reflected argument version)."""
            return GFElement(other ^ self.value, self.fp)
        ...
        def __mul__(self, other):
            ...
            try:
                assert self.field is other.field, "Fields must be identical"
                return GFElement(self.value * other.value, self.r + other.r)
            except AttributeError:
                return GFElement(self.value * other, self.r)

        __rmul__ = __mul__

        def __pow__(self, exponent):
            """Exponentiation."""
            return GFElement(pow(self.value, exponent, self.modulus), exponent*self.r)

        def __neg__(self):
            """Negation."""
            return GFElement(-self.value, self.fp)
    ...
    GFElement.resolution = resolution
