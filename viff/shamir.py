...
def recombine(shares, x_recomb=0):
    ...
    try:
#BS        vector = _recombination_vectors[key]
        vector = _recombination_vectors[xs]
    except KeyError:
        vector = []
#BS        for i, x_i in enumerate(xs):
        for i, x_i in enumerate(xs[:-1]):
            factors = [(x_k - x_recomb) / (x_k - x_i)
#BS                       for k, x_k in enumerate(xs) if k != i]
                       for k, x_k in enumerate(xs[:-1]) if k != i]
#BS            vector.append(reduce(operator.mul, factors))
            vector.append(reduce(operator.mul, factors, 1))
#BS        _recombination_vectors[key] = vector
        _recombination_vectors[xs] = vector
#BS    return sum(map(operator.mul, ys, vector))
    sum = 0
...
def verify_sharing(shares, degree):
    ...
    for i in xrange(degree+1, len(shares)+1):
