...
class Player:
    ...
    def __init__(self, myid, host, port, pubkey, seckey=None, keys=None, dealer_keys=None):
        """Initialize a player."""
        self.id = myid
        self.host = host
        self.port = port
...
def load_config(source):
    ...
    for player in config:
        ...
        if 'prss_keys' in config[player]:
            ...
            players[myid] = Player(myid, host, port, pubkey, seckey, keys, dealer_keys)

            # ID of player for which this config file was made
            owner_id = myid
        else:
            players[myid] = Player(myid, host, port, pubkey)
